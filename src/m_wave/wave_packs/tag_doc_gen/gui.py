...
from m_wave.wave_packs.tag_doc_gen.paths import TDG_INSTRUCTIONS_PATH
from m_wave.wave_packs.tag_doc_gen.controller import Controller
...
with open(TDG_INSTRUCTIONS_PATH, "r", encoding="utf-8") as f:
    INSTRUCTIONS = f.read()


class TDGFrame(WavePackFrame):
    def __init__(self, parent: tkb.Frame, app: App) -> None:
        ...
        self.controller: Controller = Controller(self.app, self.context)
        ...
        self.doc_gen_type = tkb.StringVar(value=DocGeneratorType.NONE.value)
    ...
    def _on_process_clicked(self) -> None:
        ...
        if self.gen_opt_cbox.get() == DocGeneratorType.NONE.value:
        ...
        req = ProcessRequest(
            ...
            DocGeneratorType(self.doc_gen_type.get()),
            TagFileType(self.file_type_cbox.get()),
        )
    ...
    def _build_file_type_selections(self) -> None:
        ...
        file_types = [i.value for i in TagFileType]
    ...
    def _on_file_type_selected(self, event) -> None:
        ...
        file_type = TagFileType(type_selected)

        options = [t.value for t in TAG_GEN_FILE_OPTIONS[file_type]]

        self.gen_opt_cbox.configure(values=options)
        self.gen_opt_cbox.set(DocGeneratorType.NONE.value)
        self.doc_gen_type.set(self.gen_opt_cbox.get())
